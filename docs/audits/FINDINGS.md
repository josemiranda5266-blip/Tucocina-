...
| TUC-DISC-001 | 🔴 Alta | El panel Admin carecía de un sistema automatizado para descubrir videos de cocina con métricas de calidad y sin scraping | ✅ Corregido | Implementado proveedor oficial `YouTubeDiscoveryProvider` (search.list + videos.list) con detección de idioma español (+10 pts) y filtrado culinario de +5.000 vistas |
| TUC-DISC-002 | 🔴 Alta | Riesgo de publicar automáticamente contenido no auditado o exponer claves de API de terceros en el cliente | ✅ Corregido | Clave `YOUTUBE_API_KEY` restringida al servidor; todos los candidatos se importan exclusivamente en estado `DRAFT` para auditoría manual previa |
| TUC-DISC-003 | 🟠 Media | Potencial agotamiento de cuota de la API de YouTube o solicitudes duplicadas por acciones masivas | ✅ Corregido | Agregado `createDiscoveryRateLimiter` (máx 15 req/min), deduplicación previa contra base de datos y soporte de importación en lote |
| TUC-PURGE-001 | 🔴 Alta | Acumulación potencial de videos duplicados en importación o catálogo | ✅ Corregido | Implementada depuración automática en `dbStore.purgeDuplicates()` y endpoint `POST /api/admin/videos/purge-duplicates` que mantiene la versión publicada/más antigua y elimina réplicas |
| TUC-PURGE-002 | 🔴 Alta | Videos eliminados o privatizados por creadores en la plataforma de origen seguían mostrándose en el catálogo | ✅ Corregido | Implementado `checkVideoOriginStatus` (vía YouTube API / oEmbed) con verificación en tiempo real en la vista de detalle y limpieza masiva `POST /api/admin/videos/purge-origin-deleted` |
| TUC-COMM-001 | 🟠 Media | Falta de interacción de usuarios autenticados para dejar opiniones o comentarios en recetas | ✅ Corregido | Creado componente `<VideoComments>`, subcolección `comments` en Firestore, endpoints REST autenticados (`GET`, `POST`, `DELETE`) y reglas de seguridad `firestore.rules` con validación estricta |
| TUC-MON-001 | 🟠 Media | La aplicación carecía de una arquitectura independiente y desacoplada para futura monetización publicitaria | ✅ Corregido | Diseñada capa de publicidad desacoplada en `src/components/ads/` (`AdSlot`, `AdProvider`, `adConfig`). Publicidad estrictamente DESACTIVADA (`ads.enabled = false`), sin scripts, anuncios ni impacto en red/performance |
