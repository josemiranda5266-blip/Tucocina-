...
import { ChefHat, Flame, Clock, ArrowRight, Utensils, Shield, ChevronLeft, ChevronRight, Sparkles } from 'lucide-react';
  ...
  return (
    <div className="space-y-12 pb-12">
      ...
      <section id="home-categories" className="space-y-4">
        <div className="flex items-center justify-between"><div className="flex items-center space-x-2"><Utensils className="w-5 h-5 text-amber-700" /><h2 className="text-2xl font-bold font-serif text-stone-900">Categorías Gastronómicas</h2></div><button onClick={() => onNavigate('categories')} className="flex items-center space-x-1 text-amber-700 hover:text-amber-900 text-sm font-bold"><span>Ver todas</span><ArrowRight className="w-4 h-4" /></button></div>
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-4">{categories.slice(0, 6).map((cat) => <CategoryCard key={cat.id} category={cat} onClick={(category) => onNavigate('search', `cat:${category.id}`)} />)}</div>
      </section>

      <AdSlot placement="HOME_MIDDLE" />

      <section id="home-popular" className="space-y-5">
        <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-2"><div className="flex items-center space-x-2"><Flame className="w-5 h-5 text-amber-600" /><div><h2 className="text-2xl font-bold font-serif text-stone-900">Recetas Populares</h2><p className="text-sm text-stone-500 mt-0.5">Las más vistas, más algunas recetas para descubrir algo nuevo.</p></div></div><span className="text-xs font-semibold text-stone-500">Página {popularPage} de {popularTotalPages}</span></div>

        <VideoGrid videos={popularVideos} loading={loading || popularLoading} onVideoSelect={onVideoSelect} emptyTitle="Aún no hay recetas destacadas" emptyMessage="Los administradores agregarán nuevos videos pronto." />
