...
import { Edit, Trash2, EyeOff, CheckCircle, X } from 'lucide-react';
...
export const AdminVideos: React.FC = () => {
  ...
  useEffect(() => {
    loadVideos();
    // loadVideos intentionally follows the current filter/cursor state.
  ...
  const handleToggleStatus = async (video: Video, newStatus: Video['status']) => {
    try {
      ...
      setError('Error al actualizar el estado del video.');
    }
  };
  ...
  return (
    <div className="space-y-6">
      ...
      {loading ? (
        <div className="py-12 text-center text-stone-500"><div className="w-8 h-8 border-4 border-amber-600 border-t-transparent rounded-full animate-spin mx-auto mb-3" /><p className="text-sm font-medium">Cargando catálogo...</p></div>
      ) : videos.length === 0 ? (
        <div className="py-12 text-center bg-stone-50 rounded-2xl border border-stone-200 text-stone-500">No hay videos cargados en este estado.</div>
      ) : (
        <div className="bg-white rounded-2xl border border-stone-200/80 shadow-sm overflow-x-auto">
          <table className="w-full text-left text-xs">
            <thead className="bg-stone-100 text-stone-700 font-bold border-b border-stone-200"><tr><th className="p-3">Video</th><th className="p-3">Plataforma</th><th className="p-3">Categoría</th><th className="p-3">Estado</th><th className="p-3 text-right">Acciones</th></tr></thead>
            <tbody className="divide-y divide-stone-100">
              {videos.map((v) => (
                <tr key={v.id} className="hover:bg-stone-50/80 transition-colors">
                  <td className="p-3"><div className="flex items-center space-x-3"><img src={v.thumbnailUrl} alt={v.title} loading="lazy" className="w-12 h-8 rounded object-cover shrink-0 bg-stone-200" /><div><p className="font-bold text-stone-900 line-clamp-1 max-w-xs">{v.title}</p><p className="text-[10px] text-stone-500">{v.creatorName}</p></div></div></td>
                  ...
                  <td className="p-3 text-right"><div className="flex items-center justify-end space-x-1">
        ...
        <div className="bg-white rounded-2xl max-w-lg w-full p-6 shadow-2xl relative border border-stone-200 space-y-4">
          ...
          <form onSubmit={handleSaveEdit} className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div><label className="block text-xs font-bold text-stone-700 mb-1">Categoría</label><select value={editCategory} onChange={(e) => setEditCategory(e.target.value)} className="w-full bg-stone-50 border border-stone-300 rounded-xl p-2.5 text-sm">{categories.map((c) => <option key={c.id} value={c.id}>{c.name}</option>)}</select></div>
              <div><label className="block text-xs font-bold text-stone-700 mb-1">Estado de Publicación</label><select value={editStatus} onChange={(e) => setEditStatus(e.target.value as Video['status'])} className="w-full bg-stone-50 border border-stone-300 rounded-xl p-2.5 text-sm"><option value="DRAFT">Borrador</option><option value="PENDING_REVIEW">En revisión</option><option value="PUBLISHED">Publicado</option><option value="HIDDEN">Oculto</option></select></div>
            </div>
            <div className="flex justify-end space-x-2 pt-2"><button type="button" onClick={() => setEditingVideo(null)} className="px-4 py-2 rounded-xl text-stone-600 hover:bg-stone-100 text-sm">Cancelar</button><button type="submit" disabled={saveLoading} className="bg-amber-600 hover:bg-amber-700 disabled:opacity-50 text-white font-medium px-5 py-2 rounded-xl text-sm">{saveLoading ? 'Guardando...' : 'Guardar Cambios'}</button></div>
          </form>
        </div>
      </div>}
    </div>
