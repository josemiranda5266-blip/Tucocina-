...
import { AuthProvider, CURRENT_LEGAL_VERSION, useAuth } from './context/AuthContext';
...
function readLocation(): AppLocation {
  ...
  if (path === '/contacto') return { view: 'contact', param: '' };
  ...
  if (path.startsWith('/video/')) { const id = safeDecode(path.slice('/video/'.length)); return id ? { view: 'video-detail', param: id } : { view: 'home', param: '' }; }
...
function writeLocation(view: string, param = '') {
  ...
  switch (view) {
    ...
    case 'contact': path = '/contacto'; break;
    case 'legal': path = param === 'terms' ? '/legal/terminos' : param === 'cookies' ? '/legal/cookies' : param === 'content' ? '/legal/contenido' : '/legal/privacidad'; break;
    default: path = '/';
  }
...
export const AppContent: React.FC = () => {
  ...
  const renderView = () => {
    if (location.view === 'admin') return <AdminLayout currentAdminTab={adminTab} onTabChange={handleAdminTabChange}>{adminTab === 'dashboard' && <AdminDashboard />}{adminTab === 'videos' && <><QuickVideoManager /><AdminVideos /></>}{adminTab === 'import' && <AdminImportVideo onVideoSelect={handleVideoSelect} />}{adminTab === 'reports' && <AdminReports />}</AdminLayout>;
  ...
  return <div className="min-h-screen bg-stone-100 text-stone-900 font-sans flex flex-col selection:bg-amber-500 selection:text-white">
    <Header currentView={location.view} onNavigate={handleNavigate} />
    {authError && <div className="bg-red-50 border-b border-red-200 px-4 py-3 text-red-800 text-sm flex items-center justify-between max-w-7xl mx-auto w-full mt-2 rounded-lg shadow-sm"><div className="flex items-center space-x-2"><AlertTriangle className="w-5 h-5 text-red-600 flex-shrink-0" /><div><strong className="font-semibold">Error de Autenticación:</strong> {authError}</div></div><button type="button" onClick={clearAuthError} className="p-1 text-red-600 hover:text-red-900 rounded-md hover:bg-red-100 transition-colors" title="Cerrar mensaje"><X className="w-4 h-4" /></button></div>}
    <main className="flex-1 max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8 py-8">{renderView()}</main>
    <Footer onNavigate={handleNavigate} />
    {needsLegalAcceptance && <LegalConsentModal onNavigate={handleNavigate} />}
  </div>;
};
