...
function youtubeEmbedWithApi(url: string): string {
  try {
    ...
    if (!/youtube(?:-nocookie)?\.com$/i.test(parsed.hostname)) return url;
...
export const VideoPlayer: React.FC<VideoPlayerProps> = ({ video, compact = false }) => {
  ...
  useEffect(() => {
    ...
    const handleMessage = (event: MessageEvent) => {
      ...
      // YouTube IFrame API: state 1 = PLAYING.
  ...
  const handleOriginalOpen = () => {
    track('video_open_external', { videoId: video.id });
  };
      ...
      <div id="video-player-container" className="w-full flex flex-col items-center my-2">
        <div className={`w-full ${compact ? 'max-w-[340px]' : 'max-w-[390px]'} bg-stone-950 rounded-2xl overflow-hidden shadow-2xl border border-stone-800 flex flex-col`}>
          <div className="bg-stone-900/90 px-3.5 py-2 border-b border-stone-800 flex items-center justify-between">
            {renderPlatformBadge()}
            {safeOriginalUrl && (
              <a href={safeOriginalUrl} target="_blank" rel="noopener noreferrer" onClick={handleOriginalOpen} className="text-[11px] font-medium text-stone-400 hover:text-white flex items-center space-x-1 transition-colors" title="Abrir en plataforma original">
                <span>Original</span><ExternalLink className="w-3 h-3" />
              </a>
            )}
          </div>
          <div className={`relative w-full ${compact ? 'h-[460px] max-h-[55vh]' : 'h-[580px] sm:h-[620px] max-h-[72vh]'} bg-black flex items-center justify-center overflow-hidden`}>
            {safeEmbedUrl ? renderIframe(youtubeSrc || safeEmbedUrl, video.title) : (
              <div className="text-center p-6 text-stone-400">
                ...
                {safeOriginalUrl && <a href={safeOriginalUrl} target="_blank" rel="noopener noreferrer" onClick={handleOriginalOpen} className="inline-flex items-center space-x-1.5 bg-amber-600 text-white font-medium px-4 py-2 rounded-xl text-xs shadow hover:bg-amber-500 transition-colors"><span>Ver en la plataforma original</span><ExternalLink className="w-3.5 h-3.5" /></a>}
              </div>
            )}
          </div>
        </div>
