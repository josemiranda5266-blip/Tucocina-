...
import { ChefHat, Search, Heart, Shield, LogIn, LogOut, Menu, X, User as UserIcon, Utensils, Trash2, Mail, ChevronDown } from 'lucide-react';
        ...
        <div className="flex items-center justify-between h-16">
          ...
          <nav aria-label="Navegación principal" className="hidden md:flex items-center space-x-1">
            ...
            <div className="relative" onMouseLeave={() => setCategoriesOpen(false)}>
              <button type="button" id="nav-categories-btn" aria-haspopup="menu" aria-expanded={categoriesOpen} onClick={() => setCategoriesOpen((open) => !open)} className={`flex items-center space-x-1 px-3 py-2 rounded-lg text-sm font-medium transition-colors ${currentView === 'categories' || categoriesOpen ? 'bg-amber-800 text-white font-semibold' : 'text-amber-100 hover:bg-amber-800/60 hover:text-white'}`}><Utensils className="w-4 h-4" aria-hidden="true" /><span>Categorías</span><ChevronDown className={`w-3.5 h-3.5 transition-transform ${categoriesOpen ? 'rotate-180' : ''}`} aria-hidden="true" /></button>
              {categoriesOpen && <div role="menu" className="absolute left-0 top-full pt-2 w-52 z-50"><div className="rounded-xl bg-stone-900 border border-amber-800/60 shadow-2xl p-1.5"><button type="button" role="menuitem" onClick={openCategories} className="w-full text-left px-3 py-2.5 rounded-lg text-sm font-semibold text-amber-100 hover:bg-amber-800/70 hover:text-white">Ver todas las categorías</button><div className="my-1 border-t border-stone-700" /><p className="px-3 py-1.5 text-[10px] uppercase tracking-wider font-bold text-amber-400">Explorá por tipo</p><button type="button" role="menuitem" onClick={() => { setCategoriesOpen(false); onNavigate('search', 'cat:cat-carnes'); }} className="w-full text-left px-3 py-2 rounded-lg text-sm text-stone-200 hover:bg-stone-800 hover:text-white">Carnes</button><button type="button" role="menuitem" onClick={() => { setCategoriesOpen(false); onNavigate('search', 'cat:cat-pollo'); }} className="w-full text-left px-3 py-2 rounded-lg text-sm text-stone-200 hover:bg-stone-800 hover:text-white">Pollo</button><button type="button" role="menuitem" onClick={() => { setCategoriesOpen(false); onNavigate('search', 'cat:cat-pastas'); }} className="w-full text-left px-3 py-2 rounded-lg text-sm text-stone-200 hover:bg-stone-800 hover:text-white">Pastas</button><button type="button" role="menuitem" onClick={() => { setCategoriesOpen(false); onNavigate('search', 'cat:cat-arroces'); }} className="w-full text-left px-3 py-2 rounded-lg text-sm text-stone-200 hover:bg-stone-800 hover:text-white">Arroces</button></div></div>}
            </div>
