      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16">
          ...
          <div className="hidden md:flex items-center space-x-3">
            ...
            {user ? <div className="flex items-center space-x-2.5 bg-amber-950/40 py-1 px-3 rounded-full border border-amber-800/60">{user.photoURL ? <img src={user.photoURL} alt={user.displayName || 'Usuario'} className="w-7 h-7 rounded-full border border-amber-400/50 object-cover" /> : <div className="w-7 h-7 rounded-full bg-amber-700 flex items-center justify-center text-xs font-bold text-white"><UserIcon className="w-4 h-4" aria-hidden="true" /></div>}<div className="flex flex-col"><span className="text-xs font-medium text-amber-100 max-w-[130px] truncate leading-tight">{user.displayName || user.email}</span>{isAdmin && <span className="text-[9px] font-bold uppercase tracking-wider text-amber-300">Administrador</span>}</div><button type="button" id="auth-signout-btn" onClick={signOut} title="Cerrar sesión" aria-label="Cerrar sesión" className="p-1 hover:bg-amber-800 text-amber-300 hover:text-amber-100 rounded-full transition-colors ml-1"><LogOut className="w-4 h-4" /></button></div> : <button type="button" id="auth-google-login-btn" onClick={signInWithGoogle} className="flex items-center space-x-2 bg-amber-500 hover:bg-amber-400 text-amber-950 px-4 py-2 rounded-lg font-semibold text-sm transition-all shadow-md active:scale-95"><LogIn className="w-4 h-4" aria-hidden="true" /><span>Ingresar con Google</span></button>}
          </div>
