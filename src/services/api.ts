...
async function fetchWithAuth(url: string, init?: RequestInit): Promise<Response> {
  ...
  if ((res.status === 401 || res.status === 403) && auth.currentUser) {
    try {
      headers.set('Authorization', `Bearer ${await auth.currentUser.getIdToken(true)}`);
      res = await fetch(url, { ...init, headers });
    } catch { /* return original response */ }
  }
  return res;
}

async function parseApiError(res: Response, fallback: string): Promise<Error> {
  try { const data = await res.json(); return new Error(data?.error?.message || data?.message || fallback); }
...
export const api = {
  ...
  async checkIsFavorite(videoId: string): Promise<boolean> { const res = await fetchWithAuth(`/api/favorites/check/${encodeURIComponent(videoId)}`); if (!res.ok) return false; const data = await res.json(); return !!data.isFavorite; },
  ...
  async adminImportVideo(url: string): Promise<Video> { const res = await fetchWithAuth('/api/admin/videos/import', { method: 'POST', body: JSON.stringify({ url }) }); if (!res.ok) { if (res.status === 409) { try { const duplicateData = await res.json(); if (duplicateData?.video) return duplicateData.video; } catch { /* ignore */ } } throw await parseApiError(res, 'Error al importar el video'); } return (await res.json()).video; },
  async adminImportVideosBulk(urls: string[]): Promise<Array<{ url: string; status: 'IMPORTED' | 'DUPLICATE' | 'FAILED'; videoId?: string; title?: string; categoryId?: string; tags?: string[]; error?: string }>> { const res = await fetchWithAuth('/api/admin/videos/import-bulk', { method: 'POST', body: JSON.stringify({ urls }) }); if (!res.ok) throw await parseApiError(res, 'Error al importar videos'); return (await res.json()).results; },
  ...
  async adminGetMetrics(): Promise<{ totalVideos: number; publishedVideos: number; pendingVideos: number; hiddenVideos: number; openReports: number; totalUsers: number }> { const res = await fetchWithAuth('/api/admin/metrics'); if (!res.ok) throw await parseApiError(res, 'Error al consultar métricas del sistema'); return res.json(); },
  async adminGetAnalytics(days = 7): Promise<{ periodDays: number; totals: Record<string, number>; daily: Array<Record<string, number | string>>; countries: Array<{ id: string; count: number }>; devices: Array<{ id: string; count: number }>; topVideos: Array<{ id: string; count: number }>; topSearches: Array<{ query: string; count: number }> }> { const res = await fetchWithAuth(`/api/admin-analytics/summary?days=${days}`); if (!res.ok) throw await parseApiError(res, 'Error al cargar analíticas'); return res.json(); },
