...
async function readCounts(parent: FirebaseFirestore.DocumentReference, collection: string) {
...
router.get('/summary', async (req: AuthenticatedRequest, res: Response) => {
  ...
  try {
    ...
    for (let offset = days - 1; offset >= 0; offset -= 1) {
      ...
      const eventRows = await readCounts(doc, 'events');
      const countryRows = await readCounts(doc, 'countries');
      const deviceRows = await readCounts(doc, 'devices');
      const videoRows = await readCounts(doc, 'videos');
      const searchRows = await readCounts(doc, 'searches');
    ...
    const topVideos = [...videos.values()].sort((a, b) => b.count - a.count).slice(0, 10);
    const topSearches = [...searchTerms.values()].sort((a, b) => b.count - a.count).slice(0, 10);

    return res.json({
      periodDays: days,
      totals: { visitors, sessions, pageViews, videoViews, videoPlays, searches, favorites, shares, signUps, logins },
      daily,
